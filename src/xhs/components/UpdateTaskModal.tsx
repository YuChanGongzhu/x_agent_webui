...
// Props for the UpdateTaskModal component
interface UpdateTaskModalProps {
  visible: boolean;
  onClose: () => void;
  onFinish: (values: any) => void;
  onRefresh?: () => void;
  initialTemplate?: TaskTemplate; // 传入的模板数据用于编辑
}
...
const UpdateTaskModal: React.FC<UpdateTaskModalProps> = ({
  ...
  const [template, setTemplate] = useState<TaskTemplate | null>(initialTemplate || null);
  ...
  useEffect(() => {
    if (template && visible) {
      console.log("模版数据", template);
      
      // 填充表单数据
      const initialValues = {
        targetEmail: template.userInfo,
        keyword: template.keyword,
        noteCount: template.max_notes,
        noteType: template.note_type,
        timeRange: template.time_range,
        sortBy: template.sort_by,
        profileSentence: template.profile_sentence,
        userProfileLevel: template.intent_type
      };
      ...
      // 如果有模板 IDs，可以加载相关的评论模板
      // 这里可能需要另外的API调用来获取评论模板内容
    }
  }, [template, visible, form, updateFormData, setCurrentStep]);
  ...
  const updateTaskTemplate = async () => {
    ...
    try {
      ...
      const content = {
        userInfo: templateContent.targetEmail || template?.userInfo,
        keyword: templateContent.keyword || template?.keyword,
        max_notes: templateContent.noteCount || template?.max_notes,
        note_type: templateContent.noteType || template?.note_type,
        time_range: templateContent.timeRange || template?.time_range,
        sort_by: templateContent.sortBy || template?.sort_by,
        profile_sentence: templateContent.profileSentence || template?.profile_sentence,
        template_ids: templateIds.length > 0 ? templateIds : template?.template_ids,
        intent_type: templateContent.userProfileLevel || template?.intent_type,
      };
      console.log("🔍 更新参数:", content);
      const response = await updateTaskTemplateAPI(template!.id, content);
      ...
      if (response.code === 0) {
        ...
        // 使用相同的 key 更新消息为错误状态
        message.error({
          content: response.message || "更新任务模板失败",
          key: messageKey,
          duration: 2,
        });
      }
    ...
    >
      ...
      <div className="flex justify-end p-2 border-t border-gray-100">
        <div className="flex space-x-2">
          ...
          <Button
            type="primary"
            onClick={currentStep === "回复模板" ? updateTaskTemplate : handleNextStep}
            className="rounded"
            style={{
